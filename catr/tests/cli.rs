...
fn bustle_stdin() -> TestResult {
    let expected_file = "tests/expected/the-bustle.txt.stdin.out";
    run_stdin(BUSTLE, &["-"], expected_file)
}

#[test]
fn bustle_stdin_n() -> TestResult {
    let expected_file = "tests/expected/the-bustle.txt.n.stdin.out";
    let args = &["-n", "-"];
    run_stdin(BUSTLE, args, expected_file)
}

#[test]
fn bustle_stdin_b() -> TestResult {
    let expected_file = "tests/expected/the-bustle.txt.b.stdin.out";
    let args = &["-b", "-"];
    run_stdin(BUSTLE, args, expected_file)
}
...
#[test]
fn all_n() -> TestResult {
    run(&[FOX, SPIDERS, BUSTLE, "-n"],
    "tests/expected/all.n.out")
}

#[test]
fn all_b() -> TestResult {
    run(&[FOX, SPIDERS, BUSTLE, "-b"],
